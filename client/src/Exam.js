...
function PlayerBox({ socket, player }) {
    return (
        <div className={`player-box ${player.id === socket.id ? "user" : ""}`}>
            ...
            <div className="player-box-status">
                {player.status == 0 ? "Answering" : "Submitted"}
            </div>
        </div>
    );
}
    ...
    return (
        <nav className="status-bar">
            <div className="time-remaining-wrapper">
                {submitted ? "Time Used" : "Time Remaining"}
                <span className="time-remaining">{parseTime(time)}</span>
            </div>
            {players.map((p) => {
    ...
    return (
        <article id={`problem-${index + 1}`} className="problem">
            <div className="problem-title">
                ...
                {submitted ? (
                    <a
                        className="problem-tooltip"
                        href={solution?.link}
                        target="_blank"
                    >
                        <i class="fas fa-info-circle"></i>
                        <div className="problem-tooltip-name">
                            {solution?.name}
                        </div>
                    </a>
                ) : (
            ...
            {results?.map((result) => {
                return (
                    <div
                        className={
                            "result-card " +
                            (result?.id === socket.id ? "isUser" : "")
                        }
                    >
                        <div className="result-name">{result?.name}</div>
                        <div className="result-time">
                            Time Used: {parseTime(result?.time)}
                        </div>
